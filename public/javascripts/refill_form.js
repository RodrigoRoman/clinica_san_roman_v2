            ...
            if(response.secondarySort == 'serviceData.name'){
                ...
                tablesContent += (`
                                        <tr>  
                                            <td>${item.service.name}</td> 
                                            <td>${item.service.principle}</td>  
                                            <td>${makeDMY(new Date(item.service.expiration))}</td>  
                                            <td>${item.amount}</td>  
...
receiptContent += `           _____________________\n\n`;
    ...
    if (secondarySort == 'serviceData.name') {
        ...
        transactions.items.forEach(function (item, index) {
            receiptContent += `${(item.service.name + ' - '+item.service.principle).padEnd(15)} \n ${makeMY(new Date(item.service.expiration)).padEnd(3)} ${item.amount.toString().padEnd(3)} ${makeDMYHour(new Date(item.consumtionDate)).padEnd(5)} ${abbreviateSentence(item.addedBy.username).padEnd(5)} ${abbreviateSentence(item.patient.name).padEnd(5)} ${abbreviateSentence(item.location).padEnd(5)}\n`;
    });
}
receiptContent += ` \n\n`
